...
import { AddPlatComponent, DialogData } from '../plat/add-plat/add-plat.component';
...
export class MainBoissonGestionComponent implements OnInit {
            ...
            this.TypeServices.getTypesData().subscribe({
              next: (data) => {
                ...
                allproducts.forEach((produit: IProduit) => {
                  if (produit.supplement == false && this.getTypeNameFromId(produit.typeDeProduitId)  == "boisson") {
                    this.produits.push(produit)
                  }
                });
                console.log(this.produits)
                this.toast.info({
                  detail: this.produits.length + " boisson(s) trouvées",
                  summary: "Found",
                  duration: 1500
                });
  ...
  public ajouter(): void {
    let dialogRef = this.dialog.open<AddPlatComponent, DialogData, IProduit>(AddPlatComponent, {
      height: '550px',
      width: '400px',
      data: {type:"boisson",produit:null}
    });
  ...
  public modifier(id: number): void {
    ...
    let dialogRef = this.dialog.open<AddPlatComponent, DialogData, IProduit>(AddPlatComponent, {
      height: '550px',
      width: '400px',
      data: {type:"boisson",produit:produit}
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (!result) return;
