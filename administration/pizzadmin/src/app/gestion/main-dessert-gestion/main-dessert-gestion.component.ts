...
export class MainDessertGestionComponent implements OnInit {
            ...
            this.TypeServices.getTypesData().subscribe({
              next: (data) => {
                ...
                allproducts.forEach((produit: IProduit) => {
                  if (produit.supplement == false && this.getTypeNameFromId(produit.typeDeProduitId)  == "dessert") {
                    this.produits.push(produit)
                  }
                });
                console.log(this.produits)
                this.toast.info({
                  detail: this.produits.length + " dessert(s) trouvés",
                  summary: "Found",
                  duration: 1500
                });
  ...
  public ajouter(): void {
    let dialogRef = this.dialog.open<AddPlatComponent, DialogData, IProduit>(AddPlatComponent, {
      height: '550px',
      width: '400px',
      data: {type:"dessert",produit:null}
    });
  ...
  public modifier(id: number): void {
    ...
    let dialogRef = this.dialog.open<AddPlatComponent, DialogData, IProduit>(AddPlatComponent, {
      height: '550px',
      width: '400px',
      data: {type:"dessert",produit:produit}
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (!result) return;
