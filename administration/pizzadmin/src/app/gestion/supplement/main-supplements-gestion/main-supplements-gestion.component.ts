...
export class MainSupplementsGestionComponent implements OnInit {
          ...
          next: (data) => {
            ...
            this.TypeServices.getTypesData().subscribe({
              next: (data) => {
                ...
                this.toast.info({
                  detail: this.produits.length + " supplément(s) trouvés",
                  summary: "Found",
                  duration: 1500
                });
  ...
  public delete(id: number): void {
    ...
    if (window.confirm('Voulez-vous vraiment supprimer ce supplément? (Supplément pour ' + this.getTypeNameFromId(prod.typeDeProduitId) + ' : "' + prod.nom + '")')) {
